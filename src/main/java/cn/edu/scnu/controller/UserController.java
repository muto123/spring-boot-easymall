...
public class UserController {
	...
	public SysResult login(String userName,String userPassword,HttpSession session,
		...
		if(user==null) {
			...
			CookieUtils.setCookie(request, response, "EM_TICKET", user.getUserId());//ticket存userId
			return SysResult.ok();
		}else {
	...
	public SysResult logout(HttpSession session,HttpServletRequest request,
	...
	public SysResult userSave(String userName,String userPassword,String userNickName,String userEmail) {
		//1.username是否被注册
		User user=User.builder().userName(userName).userPassword(userPassword).userNickname(userNickName).userEmail(userEmail).build();
